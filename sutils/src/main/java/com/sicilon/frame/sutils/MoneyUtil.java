...
public class MoneyUtil {
	     ...
	     * @return
	     */
	    public static String toChineseMoney(double d) {
	        if (d > Long.MAX_VALUE)
	    ...
	    private static void fractionProcess(double d, StringBuffer result) {
	            ...
	            int fraction = (int)((d * 100) % 100);	   
	            	for (int i = 1; i < N.length; i++) {
	            		//如果当前传入的result最后两位为X元（X代表1-9）则不需要再加零
						if (result.substring(result.length()-2).equals(N[i]+"元")) {
							p=1;
							break;
						}else{
							p=0;
	            	...
	            	//如果没有小数
	            	if (fraction/10==0&&fraction%10==0) {
						result.append("整");
						p=-1;
					}	
	            	//如果最后两位为X元
	            	if (p==1) {
		            	...
		            	//如果没有分
		            	if (fraction/10!=0&&fraction%10==0) {
							result.append(N[fraction / 10] + "角整");
						}
		            	//角分均有
		            	if (fraction/10!=0&&fraction%10!=0) {
