...
import com.sicilon.frame.sutils.enums.DateTypeEnum;
...
public class DateTimeUtil implements Serializable {
	...
	public static Date getNowTime(Integer dateType) {
		if (DateTypeEnum.UTC.getValue() == dateType) {
	...
	/**
	 * 获取当前时间的下一天
	 * @param currntDate 当前日期
	 * @return 当前时间的下一天
	 */
    public static Date getNextDay(Date currntDate){
    	Calendar calendar = Calendar.getInstance();
     ...
     * @return 当前时间的上一天
     */
    public static Date getLastDay(Date currntDate){
    	Calendar calendar = Calendar.getInstance();
    	calendar.setTime(currntDate);
    	calendar.add(Calendar.DAY_OF_MONTH,-1);
    	return calendar.getTime();
